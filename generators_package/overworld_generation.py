...
def poisson_disk_sampling(
    ...
    def generate_candidate():
        ...
        random_point = random.choice(active_points) # Choose a random point in the active points
        current_point_valid_candidates = 0 # Number of valid 'candidates'
        for i in range(POISSION_K_VALUE): # K value is the number 
            valid = True
            while True:
                random_distance = random.uniform(POISSON_RADIUS, 2 * POISSON_RADIUS) # Select a random distance
                theta = random.uniform(0, 2 * math.pi) # Create a random angle
                random_candidate_position = (
                    random_point[0] + (random_distance * math.cos(theta)), # Travel a random distance along the x direction
                    random_point[1] + (random_distance * math.sin(theta)), # Travel a random distance along the y direction
                )
                if (
        ...
        if current_point_valid_candidates == 0:
            active_points.remove(random_point) # If there is no more active points remove the generated points

    while len(active_points) != 0:
        generate_candidate()
    for point in points:
        map_[point[0]][point[1]] = debug_character # Marks all poi points with a debug character (default x)
    return map_, points
...
class Town:
    ...
    def __init__(
        ...
        self.sizes = {"Hamlet": 10, "Village": 20} # Sizes of towns with their names (Hamlet / village)
        ...
        self.house_positions = list({(int(p[0]), int(p[1])) for p in self.house_positions})
...
class OverworldGeneration:
    ...
    def __init__(
        ...
        self.debugger.write(f"Generating overworld with coordinates {self.coordinates}") # Debug statement
    ...
    def add_town(self, coordinates):
        ...
        for i, row in enumerate(town.town_layout):
            for j, ele in enumerate(row):
                if (town.coordinates[0] + i < self.map_size and town.coordinates[1] + j < self.map_size):
                    if ele not in [self.grass_char]:
                        self.map[town.coordinates[0] + i][town.coordinates[1] + j] = ele
                        if ele in [' S ', ' I ']:
                            self.building_info[(town.coordinates[0] + i, town.coordinates[1] + j)] = ele
